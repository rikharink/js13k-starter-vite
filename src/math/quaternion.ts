import { DEGREE_TO_RADIAN, EPSILON } from './const';
import { cross, dot, length, Vector3, normalize as normalize_vector3 } from './vector3';

export type Quaternion = [x: number, y: number, z: number, w: number];
export function set(out: Quaternion, x: number, y: number, z: number, w: number): Quaternion {
  out[0] = x;
  out[1] = y;
  out[2] = z;
  out[3] = w;
...
export function copy(out: Quaternion, a: Quaternion): Quaternion {
  out[0] = a[0];
  out[1] = a[1];
  out[2] = a[2];
  out[3] = a[3];
...
export function multiply(out: Quaternion, a: Quaternion, b: Quaternion): Quaternion {
  const ax = a[0],
    ay = a[1],
    az = a[2],
    aw = a[3];
 ...
 */
export function from_euler(out: Quaternion, x: number, y: number, z: number): Quaternion {
 ...
 */
export function from_axis(out: Quaternion, axis: Vector3, angle: number): Quaternion {
  const half = angle / 2;
...
export function lerp(out: Quaternion, a: Quaternion, b: Quaternion, t: number): Quaternion {
  const ax = a[0];
  const ay = a[1];
  const az = a[2];
  const aw = a[3];
...
export function get_axis(out_axis: Vector3, q: Quaternion): number {
